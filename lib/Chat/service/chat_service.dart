...
class ChatService{
  ...
  Future<List<MessageModelChats>> getallmessage({required String to,required String from})async{
    List<MessageModelChats> allmessages=[];
try {
 http.Response response=await http.post(Uri.parse(getallusermessagesurl),headers: {
...
"to":to
 ...
 if(response.statusCode==200){
  for (int i = 0; i < jsonDecode(response.body).length; i++) {
    allmessages.add(MessageModelChats.fromJson(jsonEncode(jsonDecode(response.body)[i])));
  }
 }
 return allmessages;
} catch (e) {
...
return allmessages;
  ...
  Future<NewUserModel> getuserdata(String email)async{
    ...
    try {
       ...
       http.Response res=await http.get(Uri.parse("$getuserdataurl/$email"),headers:{
        'Content-Type':'Application/json'
      });
      print(res.body);
      if(res.statusCode==200){
    ...
    try {
      ...
      http.Response res = await http.post(Uri.parse(createuserchats),
          ...
          body: jsonEncode({
            ...
            "senduserid": "12345",
            "sendusername": "Free Risthey wala"
          }));
      print(res.body);
      if (res.statusCode == 200) {
