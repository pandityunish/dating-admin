...
class User extends Equatable {
  ...
  User({
    ...
    this.videoName = "",
    this.verifiedStatus = "",
    this.latitude = 30.170235,
    this.longitude = 76.867851,
  });
  ...
  static User fromSnapshot(DocumentSnapshot snap) {
    User user = User(
        About_Me: snap['About_Me'],
        uid: snap['uid'],
        puid: snap['puid'],
        Diet: snap['Diet'],
        Disability: snap['Disability'],
        Drink: snap['Drink'],
        Education: snap['Education'],
        Height: snap['Height'],
        Income: snap['Income'],
        Partner_Prefs: snap['Partner_Prefs'],
        Smoke: snap['Smoke'],
        ...
        gender: snap['gender'],
        KundaliDosh: snap['KundaliDosh'],
        MartialStatus: snap['MartialStatus'],
        ...
        dob: snap['dob'],
        status: snap['status'],
        verifiedStatus: snap['verifiedStatus'],
...
class SavedPref {
  ...
  isEmpty() {
    if (this.AgeList.isEmpty &&
        this.ReligionList.isEmpty &&
        this.KundaliDoshList.isEmpty &&
        this.MaritalStatusList.isEmpty &&
        this.dietList.isEmpty &&
        this.DrinkList.isEmpty &&
        this.SmokeList.isEmpty &&
        this.DisabilityList.isEmpty &&
        this.HeightList.isEmpty &&
        this.EducationList.isEmpty &&
        this.ProfessionList.isEmpty &&
        this.IncomeList.isEmpty &&
        this.LocatioList[0].isEmpty &&
        this.LocatioList[1].isEmpty &&
        this.LocatioList[2].isEmpty) {
      return true;
    } else {
      return false;
    }
...
class ExternalFilefunction {
  downloadImage(url, imageName) async {
    var newPath;
    if (await Permission.storage.request().isGranted) {
      ...
      newPath = directory!.path + '/${imageName}.jpg';
      ...
      SharedPref().save(
          "imgdict", imgDict.imageDictionary!.map((p) => p.toJson()).toList());
    } else {
      print("no permission");
      // permissionResult = await SimplePermissions.requestPermission(
      //     Permission.WriteExternalStorage);
      // downloadImage();
      newPath = "noPermission";
    }
    return newPath;
  }

  static deleteFileContents(Directory directory) async {
    directory.listSync().forEach((FileSystemEntity entity) {
      if (entity is File) {
        entity.deleteSync();
      } else if (entity is Directory) {
        deleteFileContents(entity);
        entity.deleteSync();
      }
    });
  }
