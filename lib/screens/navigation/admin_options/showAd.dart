...
import '../../profie_types/advertisment_types/user_ads.dart';
...
class ShowAd extends StatefulWidget {
  ...
  const ShowAd({Key? key, required this.newUserModel}) : super(key: key);

  @override
  State<ShowAd> createState() => _ReligionState();
}

class _ReligionState extends State<ShowAd> {
  int value = 0;
  List<dynamic> ads = [
                ...
                itemBuilder: (context, index) {
                  return Card(
                    child: ListTile(
                      onTap: () {
                        if (index == 102 ||
                          ...
                          SearchProfile().addtoadminnotification(
                              ...
                              title:
                                  "${userSave.displayName} SEEN  ${widget.newUserModel!.name.substring(0, 1).toUpperCase()} ${widget.newUserModel!.surname..toLowerCase()} ${widget.newUserModel!.puid} ADVERTISEMENT-${ads[index]["id"]}",
                      ...
                      title: Text("Advertisement-${ads[index]["id"]}"),
